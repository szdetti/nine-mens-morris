...
void Game::setWinner(Player& newWinner) {
    winner = &newWinner;
}
...
Used to determine if a new move has resulted in a mill. */
...
/* Check if both of the vectors in a nested vector contain field names that hold pieces belonging to the 
same player (meaning they have achieved a mill). If any of them contains field names that form a mill, 
set the corresponding pieces flag to indicate they are in a mill. */
...
/* Recursively check that fields in a vector contain pieces beloning to the same player by comparing the 
...
/* Check that the selected field is empty by determining the colour of the piece in the field. Return true if 
the colour of the piece is Colour::reset, which is the colour of default piece objects that act as placeholders 
in  empty fields. */
...
/* Check if field associated with field name selected by player contains a piece that belongs to the appropriate player. 
Return false if colour of piece on field mathes colour of current player (they chose their own piece to remove). */
...
/* Called in phase 1 of the game, when players place their pieces on the board. Retrieve the field (pointer) 
associated with the fieldname provided by the player, set the field's piece field to a pointer pointing to 
(the memory address of) a piece object associated with the current player making the move. Keep track of 
the current player's pieces by updating the vectors representing the status of the pieces 
(initial - yet to be placed on the board, on board - as named). Update the board's vector that keeps track 
of empty fields by removing the shared_ptr of the field holding the newly placed piece.*/
void Game::addPiece(FieldName fn) {
        ...
        emptyFields.erase(
            std::remove_if(
                emptyFields.begin(),
                emptyFields.end(),
                [&f](std::shared_ptr<Field>& field) {return field == f;}
            ),
            emptyFields.end()
        );
}
...
void Game::removePiece(FieldName fn) {
        ...
        getBoard().getEmptyFields().push_back(f);

}

/* Called in phase 2 of the game when users have placed all their pieces on the board and can now move them
between eligible fields. Similarly to the functions that add and remove a piece, retrieve the field pointers 
associated with the fieldnames provided by the user and set the piece fields of the field objects to the
appropriate piece pointer - a default piece for the field being moved from fields and the current player's
piece for the field being moved to. */
void Game::movePiece(FieldName fromFn, FieldName toFn) {
        ...
        emptyFields.erase(
            std::remove_if(
                emptyFields.begin(),
                emptyFields.end(),
                [&toField](std::shared_ptr<Field>& field) {return field == toField;}
            ),
            emptyFields.end()
        );
        emptyFields.push_back(fromField);
